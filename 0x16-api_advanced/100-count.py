...
def count_words(subreddit, word_list, hot_object={}, start=True, after=''):
    ...
    if res.status_code == requests.codes.ok:
        ...
        count_words(subreddit, word_list, hot_object, start=False, after=next_page)
    elif result is None:
        return None
    if start:
        if hot_object:
            sorted_object = {key: hot_object[key] for key in sorted(hot_objects) if hot_object[key] != 0}
            print(sorted_object)
        else:
            print('i got here')
